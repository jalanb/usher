...
def test_server_naming(docker_setup):
    ...
    assert (
        "docker-usher_server-1" in container_names
    ), f"server is not running: {container_names}"
...
def test_trackers_naming(docker_setup):
    ...
    container_names = [container.name for container in containers]
    assert (
        "docker-usher_tracker-1" in container_names
    ), f"tracker is not running: {container_names}"
...
def test_basic_connectivity(docker_setup):
    ...
    ), f"Client could not connect to the server. Status: {response.status_code}"
...
def test_health_check_endpoint(docker_setup):
    ...
    assert response.json() == {
        "status": "ok"
    }, f"Health check response is incorrect: {response}"
